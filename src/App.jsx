...
export default function App() {
  ...
  return (
    <div style={{ fontFamily: "sans-serif", maxWidth: 960, margin: "0 auto", padding: 20 }}>
      ...
      {duplicateNotice && (
        <div
          style={{ background: "#fff3cd", padding: 8, marginBottom: 12, fontSize: 14 }}
      ...
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        <thead>
          <tr style={{ textAlign: "left", borderBottom: "1px solid #ccc" }}>
            <th style={{ padding: 8 }}>Arquivo original</th>
            <th style={{ padding: 8 }}>Nome sugerido</th>
        ...
        <tbody>
          {filtered.map((d) => (
            <tr key={d.id} style={{ borderBottom: "1px solid #eee" }}>
              <td style={{ padding: 8, color: "#888" }}>{d.originalName}</td>
              <td style={{ padding: 8 }}>{d.suggestedName || "-"}</td>
              <td style={{ padding: 8 }}>{d.type || "-"}</td>
              <td style={{ padding: 8 }}>{statusLabel[d.status]}</td>
              <td style={{ padding: 8 }}>
                {(d.status === "review" || d.status === "done") && (
                  <button onClick={() => openReview(d)}>Ver</button>
                )}
              </td>
            </tr>
          ))}
        ...
        >
          ...
          >
            ...
            <div style={{ flex: 1 }}>
              ...
              {conflict && (
                <div style={{ background: "#f8d7da", padding: 8, marginBottom: 10, fontSize: 14 }}>
                  ⚠ {conflict}
                </div>
              )}
